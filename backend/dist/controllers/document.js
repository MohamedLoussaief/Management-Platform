...
export const generatePdf = async (req, res) => {
    ...
    try {
        ...
        doc.pipe(res);
        userRequest?.requestType == "Work Certificate" ? generateWorkCertificate(doc, userData?.firstName + " " + userData?.lastName, userRequest.cin, userData?.func, userData?.registrationDate, userRequest.certificateReason) : leaveDocument(doc, userData?.firstName, userData?.lastName, userDepart?.departName, userRequest?.leaveType, userRequest?.startDate, userRequest?.endDate, userRequest?.returnDate, userRequest?.substituteName);
    }
    catch (error) {
        res.status(500).json({ msg: "Server error" });
    }
